...
- **Ringkasan per-ranting** di `/console/ranting/sensus` untuk role `[INA]/[JIA] BPH Ranting` (key non-delegable **`sensus-ranting`**): daftar mahasiswa kampus ranting itu saja + status lengkap/belum + apakah kartu diunggah. Tanpa paspor, tanpa halaman detail, tanpa ekspor. Filter kampus lewat pencocokan `sensus_profiles.university` (lihat `src/lib/rantings.ts`; NUIST sudah dipetakan, JSAHVC masih TODO). `[INA]/[JIA] Anggota Ranting` = tanpa akses console. Role dikenali lewat namanya di `resolveAdminScope()` (`src/auth.ts`); di-seed `src/db/seed.ts` / `drizzle/0034_ranting_roles.sql`.
