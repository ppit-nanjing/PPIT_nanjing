...
- **`npm run build` passing does NOT mean the live Neon schema is in sync.** Next.js's build only statically analyzes routes — it never actually executes a query against the adapter tables (`accounts`/`sessions`/`verification_tokens`), so schema.ts drifting ahead of what's actually pushed to Neon (via `npx drizzle-kit push --force`) stayed invisible through many green builds and only surfaced as a live 500 (`NeonDbError 42P01: undefined_table`) the first time someone actually completed Google sign-in in production. Fixed 2026-08-15 by re-running `drizzle-kit push`. **Lesson: re-run `drizzle-kit push` after every schema.ts change, immediately, not just once at project start** — don't rely on a passing build as a signal that the DB is caught up.
