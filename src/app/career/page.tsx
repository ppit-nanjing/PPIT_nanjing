...
function typeLabel(t: T, key: string): string {
...
export async function generateMetadata() {
  const { t } = await getT();
  return { title: t("meta.careerTitle"), description: t("meta.careerDesc") };
}
  ...
  return (
    <div className="min-h-screen bg-background text-on-background">
      ...
      <header className="max-w-2xl mx-auto px-[var(--spacing-container-padding)] pt-16 pb-10 text-center flex flex-col items-center">
        <h1 className="text-display-hero-mobile md:text-display-hero text-on-background mb-4">{t("career.title")}</h1>
      ...
      <main className="max-w-[var(--container-max)] mx-auto px-[var(--spacing-container-padding)] pb-24 flex flex-col gap-16">
        <section>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-headline-md text-on-background flex items-center gap-2">
              <Briefcase size={20} className="text-primary-container" /> {t("career.latestOpportunities")}
            </h2>
            ...
            {jobs.length === 0 ? (
              <div className="flex flex-col items-center text-center py-16 bg-surface-container-lowest border border-outline-variant border-dashed rounded-xl px-6">
                ...
                <p className="text-body-md text-on-surface-variant">{t("career.noJobs")}</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {jobs.map((j) => (
                  <a
                    key={j.id}
                    href={`/jobs/${j.id}`}
                    aria-label={t("career.jobAria", { title: j.title, company: j.company })}
                  ...
                  <div className="mt-auto flex items-center justify-between border-t border-outline-variant pt-4">
                    <span className="text-label-caps text-on-surface-variant">{formatRelativeTime(j.createdAt, t)}</span>
                    <span className="flex items-center gap-1 text-label-caps uppercase text-primary-container">
                      {t("career.jobDetailLabel")} <ArrowRight size={13} className="group-hover:translate-x-1 transition-transform" />
                    </span>
                  </div>
                </a>
              ))}
        ...
        <section id="panduan">
          <h2 className="text-headline-md text-on-background mb-2">{t("career.guidesHeading")}</h2>
          <p className="text-body-md text-on-surface-variant max-w-2xl mb-6">
            ...
            {guides.length === 0 ? (
              <div className="flex flex-col items-center text-center py-16 bg-surface-container-lowest border border-outline-variant border-dashed rounded-xl px-6">
                ...
                <p className="text-body-md text-on-surface-variant">{t("career.noGuides")}</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {guides.map((g) => (
                  <a
                    ...
                    aria-label={t("career.guideAria", { title: g.title })}
                  ...
                  <div>
                    ...
                    <span className="text-label-caps uppercase tracking-wide text-primary-container">{t("career.readArticle")}</span>
                  </div>
                </a>
              ))}
            </div>
